...
public class ImagePrepare {
	...
	public static Mat formatMatSize(Mat src) {
		Mat formatMat = new Mat();
        int newRows = 300;
        int newCols = (int)((double)newRows/src.size().width * src.size().height);
        Imgproc.resize(src, formatMat, new Size(newRows, newCols));
		return formatMat;
	}
	...
	public static Mat threshold(Mat src) {
		...
		Imgproc.threshold(src, threshold, 0, 255, Imgproc.THRESH_BINARY | Imgproc.THRESH_OTSU);
		return threshold;
	}
	...
	public static Mat imageOutline(Mat src) {
		...
		Mat kernel1 = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(35, 10));
		Mat dilate = new Mat();
